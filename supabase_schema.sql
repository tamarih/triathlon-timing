...
-- Reserve (spare) participant numbers — pre-printed, assignable to late registrants
CREATE TABLE reserve_bibs (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  event_id UUID NOT NULL REFERENCES events(id) ON DELETE CASCADE,
...
CREATE INDEX idx_reserve_bibs_event ON reserve_bibs(event_id);
